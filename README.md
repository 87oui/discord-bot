...
- Googleカレンダーの予定追加・日時変更・削除を30分ごとに検知してDiscordへ通知
- 毎朝6:01にGoogleカレンダーの当日の予定をまとめて通知
...
|---|---|
| `*/30 * * * *` | 変更通知ポーリング |
| `1 21 * * *` | 当日の予定（日本時間06:01）。同期Cronと同時刻を避ける |
...
- 初回監視（syncToken もスナップショットもないとき）は、今後 365 日の予定を `familyNotifyFilter` 適用のうえ、先頭に「監視開始」を付けて追加と同じ体裁で通知する
- syncToken 失効時はスナップショット再構築のみ（通知なし）

#### 当日の予定一覧

- 監視カレンダー全体の当日予定（日本時間）
- 0 件でも「今日は予定なし」を送信
- 取得や同期の失敗は Discord には出さず、Workers のログに残す
